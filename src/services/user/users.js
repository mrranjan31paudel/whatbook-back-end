  ...
  if (requestParams && requestParams.id) {
    ...
    query.getUserProfileDetails(user.id, ownerId, function(queryResponse) {
      ...
      queryResponse = {
        ...
        isFriend:
          queryResponse.friendStatus &&
          queryResponse.friendStatus.request_status === 1
            ? true
            : false,
        isRequestSent:
          queryResponse.friendStatus &&
          queryResponse.friendStatus.request_status === 0 &&
          queryResponse.friendStatus.senderid === user.id
            ? true
            : false,
        isRequestRecieved:
          queryResponse.friendStatus &&
          queryResponse.friendStatus.request_status === 0 &&
          queryResponse.friendStatus.recieverid === user.id
            ? true
            : false
      };
...
function changeUserData(userId, changeInfo, callController) {
  if (changeInfo.type === 'name') {
    query.changeUserName(userId, changeInfo.submitData, function(
      queryResponse
    ) {
    ...
    query.getUserPassword(userId, function(encryptedPassword) {
      ...
      decryptPassword(
        changeInfo.submitData.currentPassword,
        encryptedPassword,
        function(doesMatch) {
          if (doesMatch.err) {
            return callController({
            ...
            encryptPassword(changeInfo.submitData.newPassword, function(
