...
function getUserHomeDetails(user, returnQueryResponse) {
  dbConnection.query(
    `SELECT id, name, dob, email FROM users WHERE (id='${user.id}' AND email='${user.email}')`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
...
function getUserProfileDetails(userId, ownerId, returnQueryResponse) {
  dbConnection.query(
    `SELECT id, name, dob, email FROM users WHERE (id='${ownerId}')`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
      ...
      if (userDetails && userId !== ownerId) {
        dbConnection.query(
          `SELECT senderid, recieverid, request_status FROM user_friends WHERE ((senderid='${userId}' OR senderid='${ownerId}') AND (recieverid='${userId}' OR recieverid='${ownerId}'))`,
          function(err, result) {
            if (err) {
              return returnQueryResponse({ err: err });
            }
...
function changeUserDOB(userId, newDate, returnQueryResponse) {
  dbConnection.query(
    `UPDATE users SET dob='${newDate}' WHERE id='${userId}'`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
