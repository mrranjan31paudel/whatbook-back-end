...
function getSpecificPost(userId, ownerId, postId, returnQueryResponse) {
  dbConnection.query(
    `SELECT request_status FROM user_friends WHERE ((senderid='${userId}' AND recieverid='${ownerId}') OR (senderid='${ownerId}' AND recieverid='${userId}'))`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
      ...
      if (result.length > 0 || userId === ownerId) {
        dbConnection.query(
          `SELECT user_posts.id, user_posts.userid, users.name, user_posts.date_time, user_posts.content FROM users INNER JOIN user_posts ON (users.id='${ownerId}' AND user_posts.userid='${ownerId}') WHERE (user_posts.id='${postId}')`,
          function(err, result) {
            if (err) {
              return returnQueryResponse({ err: err });
            }
...
function postUserStatus(user, postData, returnQueryResponse) {
  ...
  dbConnection.query(
    `INSERT INTO user_posts (userid, content, date_time) VALUES('${user.id}', '${postData}', '${dateTime}' )`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
...
function updateUserPost(user, postData, returnQueryResponse) {
  dbConnection.query(
    `UPDATE user_posts SET content='${postData.newPostText}' WHERE (userid='${user.id}' AND id='${postData.postId}')`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
...
function deleteUserPost(user, postData, returnQueryResponse) {
  dbConnection.query(
    `DELETE FROM user_comments WHERE (postid='${postData.postId}')`,
    function(err, result) {
      if (err) {
        return returnQueryResponse({ err: err });
      }
      dbConnection.query(
        `DELETE FROM user_posts WHERE (id='${postData.postId}' AND userid='${user.id}')`,
        function(err, result) {
          if (err) {
            return returnQueryResponse({ err: err });
          }
